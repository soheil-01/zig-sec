...
pub fn main() !void {
    ...
    const message_box_a = try sec.common.loadFunction(
        *anyopaque,
        "user32.dll",
        "MessageBoxA",
    );
    defer _ = FreeLibrary(message_box_a.h_module);

    try sec.code_injection.local.injectShellCode(message_box_a.func, shell_code);
