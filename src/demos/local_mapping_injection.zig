...
pub fn main() !void {
    ...
    const map_address = try code_injection.local.mapInject(shell_code);
    defer _ = UnmapViewOfFile(map_address);

    try code_injection.local.executeInNewThread(@ptrCast(map_address), null);
}
